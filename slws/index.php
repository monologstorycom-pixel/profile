...
foreach ($categories_db as $kat) {
    ...
    $photo_urls = array_map(fn($p) => '../' . $p, $photos_db);
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        body {
            ...
            min-height: 100vh; font-size: 14px; line-height: 1.65;
        ...
        .toggle {
            ...
            transition: border-color 0.2s, color 0.2s; font-family: 'Inter', sans-serif;
        }
        ...
        .logo-line { font-family: 'IBM Plex Mono', monospace; font-size: 11px; color: var(--accent); letter-spacing: 0.1em; display: block; margin-bottom: 10px; }
        h1 { font-size: clamp(1.8rem,5vw,2.6rem); font-weight: 600; color: var(--text-head); letter-spacing: -0.03em; line-height: 1.1; margin-bottom: 8px; }
        ...
        .section-label {
            ...
            color: var(--text-dim); display: flex; align-items: center; gap: 10px; margin-bottom: 20px;
        }
        ...
        .folder-card {
            ...
            transition: border-color 0.2s, transform 0.2s; aspect-ratio: 4/3;
        }
        .folder-card:hover { border-color: var(--accent); transform: translateY(-3px); }
        .folder-cover { width: 100%; height: 100%; object-fit: cover; display: block; transition: transform 0.4s ease; }
        ...
        .folder-info { position: absolute; bottom: 0; left: 0; right: 0; padding: 28px 12px 12px; background: linear-gradient(to top, rgba(0,0,0,0.75) 0%, transparent 100%); display: flex; align-items: flex-end; justify-content: space-between; }
        .folder-name { font-size: 13px; font-weight: 600; color: #fff; line-height: 1.2; text-shadow: 0 1px 4px rgba(0,0,0,0.5); }
        .folder-count { font-family: 'IBM Plex Mono', monospace; font-size: 10px; color: rgba(255,255,255,0.65); text-shadow: 0 1px 3px rgba(0,0,0,0.5); }
        .folder-icon-badge { position: absolute; top: 10px; left: 10px; background: rgba(0,0,0,0.45); backdrop-filter: blur(4px); border-radius: 6px; padding: 4px 7px; font-size: 11px; color: rgba(255,255,255,0.85); display: flex; align-items: center; gap: 4px; }
        ...
        /* Thumbnail wrapper — lazy load iframe via click */
        .vc-thumb {
        ...
        .vc-thumb:hover .vc-play-btn { background: rgba(255,255,255,0.3); transform: scale(1.08); }
        ...
        .gallery-title { font-size: clamp(1.4rem,4vw,2rem); font-weight: 600; color: var(--text-head); letter-spacing: -0.02em; }
        ...
        .masonry-overlay {
            position: absolute; inset: 0; background: rgba(0,0,0,0); opacity: 0;
            display: flex; align-items: center; justify-content: center;
            transition: all 0.25s; font-size: 20px; color: #fff;
        }
        ...
        /* ── LIGHTBOX ── */
        .lightbox { display: none; position: fixed; inset: 0; z-index: 999; background: rgba(0,0,0,0.93); align-items: center; justify-content: center; }
        .lightbox.open { display: flex; animation: fadeIn 0.2s ease; }
        @keyframes fadeIn { from{opacity:0} to{opacity:1} }
        .lb-wrap { position: relative; max-width: min(92vw,960px); max-height: 90vh; animation: lbUp 0.25s ease; }
        @keyframes lbUp { from{transform:scale(0.94);opacity:0} to{transform:scale(1);opacity:1} }
        .lb-img { max-width: 100%; max-height: 90vh; object-fit: contain; border-radius: 8px; display: block; }
        .lb-close { position: absolute; top: -38px; right: 0; background: none; border: none; color: #fff; font-size: 30px; cursor: pointer; opacity: 0.65; transition: opacity 0.15s; padding: 6px; }
        .lb-close:hover { opacity: 1; }
        .lb-nav { position: absolute; top: 50%; transform: translateY(-50%); background: rgba(255,255,255,0.1); border: none; color: #fff; padding: 10px 14px; cursor: pointer; font-size: 16px; border-radius: 6px; transition: background 0.2s; backdrop-filter: blur(4px); }
        ...
        .lb-counter { position: absolute; bottom: -32px; left: 50%; transform: translateX(-50%); color: rgba(255,255,255,0.55); font-size: 11px; font-family: 'IBM Plex Mono', monospace; }
        ...
        /* ── DIVIDER between foto & video ── */
        .section-divider { max-width: 900px; margin: 0 auto 0; padding: 0 24px; }
    ...
    <!-- Apply saved theme before render — no flash -->
    <script>
    (function(){
        try {
    ...
    <div class="folder-section" style="padding-bottom: 32px;">
        ...
        <div class="video-grid">
            ...
            <div class="video-card">
                <!-- Thumbnail + play button — iframe only loads on click -->
                <div class="vc-thumb" id="thumb-<?= $vid_id ?>" onclick="playVideo('<?= $vid_id ?>', '<?= htmlspecialchars($embed) ?>')">
                    ...
                    <?php else: ?>
                        <div style="width:100%;height:100%;background:#111;display:flex;align-items:center;justify-content:center">
                            <i class="fas fa-film" style="font-size:32px;color:#444"></i>
                        </div>
                    <?php endif; ?>
                    <div class="vc-play">
                        <div class="vc-play-btn"><i class="fas fa-play"></i></div>
                    </div>
                </div>
                <!-- iframe container — hidden until play clicked -->
...
function renderHome() {
    ...
    if (!categories.length) {
        grid.innerHTML = '<div class="empty-state"><i class="fas fa-folder-open"></i><p>Belum ada kategori foto.</p></div>';
        return;
    }
...
function openCategory(id) {
    ...
    if (!cat.photos.length) {
        ...
        cat.photos.forEach((url, idx) => {
            ...
            item.innerHTML = `<img src="${url}" alt="${cat.name} ${idx+1}" loading="lazy" decoding="async"><div class="masonry-overlay"><i class="fas fa-expand"></i></div>`;
...
function goHome() {
    // Pause any playing videos when going back
    document.querySelectorAll('.vc-iframe-wrap.active').forEach(el => {
        el.innerHTML = '';
        el.classList.remove('active');
        const thumb = document.getElementById(el.id.replace('iframe-', 'thumb-'));
